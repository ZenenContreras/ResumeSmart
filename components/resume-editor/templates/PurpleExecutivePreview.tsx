...
import { getPreviewStyles } from '@/lib/styles/purple-executive-styles';

interface PurpleExecutivePreviewProps {
  data: ResumeData;
}

export function PurpleExecutivePreview({ data }: PurpleExecutivePreviewProps) {
        ...
        <div style={styles.section}>
          ...
          {education.map((edu, index) => (
            <div key={index} style={styles.educationItem}>
              ...
              {edu.gpa && <p style={{ fontSize: '10pt', color: '#374151' }}>GPA: {edu.gpa}</p>}
            </div>
          ))}
        </div>
      )}
          ...
          <div>
            {Object.entries(skills).map(([category, skillList], index) => (
              <div key={index} style={{ display: 'flex', gap: '8px', marginBottom: '6px', fontSize: '10pt' }}>
                <span style={{ fontWeight: 'bold', minWidth: '100px', color: '#7c3aed', textTransform: 'capitalize' }}>
        ...
        <div style={styles.section}>
          ...
          {certifications.map((cert, index) => (
            <div key={index} style={{ marginBottom: '8px' }}>
              ...
              {cert.credentialId && (
                <p style={{ fontSize: '9pt', color: '#6b7280' }}>Credential ID: {cert.credentialId}</p>
              )}
            </div>
          ))}
        </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
            {languages.map((lang, index) => (
              <div key={index} style={{ fontSize: '10pt', color: '#374151' }}>
                <span style={{ fontWeight: 'bold', color: '#7c3aed' }}>{lang.language}</span> - {lang.proficiency}
              </div>
            ))}
          </div>
        </div>
